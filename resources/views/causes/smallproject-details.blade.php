@extends('layout')
@section('title') تفاصيل حالات التبرع للمشاريع الصغيرة@endsection
@section('main')
		...
		<div class="row justify-content-center">
			<div class="col-lg-10">
				...
				<div class="single-details">
					...
					<h2 class="my-4 text-lg ">{{$smallp->title}}</h2>
                     

                	<div class="bg-light p-4 d-flex align-items-center justify-content-between mb-4">
						<div class="cause-meta">
							<ul class="list-inline mb-0">
					        	<li class="list-inline-item"><i class="icofont-check text-color mr-2"></i>الهدف:	<span class="font-weight-bold">{{$smallp->money}} دينار</span></li>
						...
						<a href="{{url("smallproject/donate/$smallp->id")}}" class="btn btn-main rounded mainbg-color hoverbutton text-light">تبرع الان</a>
					</div>
                    @include('inc.message')


					<p class="bg-light py-5 px-5 para-space"> {{$smallp->name}}  يحتاج لمساعدة مالية لانشاء مشروعه الخاص وهو عبارة عن "{{$smallp->title}}"
                          في منطقة {{$smallp->address}} 
                          للمزيد من التفاصيل التواصل مع الرقم {{$smallp->mobile}}  </p>
